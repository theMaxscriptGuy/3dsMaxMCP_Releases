...
class ChatEngine:
    ...
    def send(self, text, provider, api_key, model=None, on_tool_call=None):
        if provider == "openai":
            return self._send_openai(text, api_key, model or MODELS["openai"], on_tool_call)
        if provider == "openrouter":
            return self._send_openrouter(text, api_key, model or MODELS["openrouter"], on_tool_call)
        return self._send_anthropic(text, api_key, model or MODELS["anthropic"], on_tool_call)

    def _send_anthropic(self, text, api_key, model, on_tool_call):
    ...
    def _send_openai(self, text, api_key, model, on_tool_call):
    ...
    def _send_openrouter(self, text, api_key, model, on_tool_call):
